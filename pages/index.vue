<template>
  ...
  >
    <div>
      <div class="text-4xl md:text-8xl">BEAT-MOTIVATOR</div>
      <a
        ...
        "
      >
        IIDX score download page (Download CSV)
      </a>
      <div class="mx-4 my-2">
        <c-file-input :file.sync="file" />
      ...
      <div class="flex justify-center items-center">
        <div class="m-2">
          <input
            ...
            name="check_12"
            id="check_12"
          /><label for="check_12">Shows only 12</label>
        </div>
        <div class="m-2">
          <input
            v-model="enable_heatmap"
            type="checkbox"
            name="enable_heatmap"
            id="enable_heatmap"
      ...
      <div
        v-html="statistics"
        class="w-72 md:w-full mx-4 my-2 overflow-x-auto"
      ></div>
      <div
        v-html="table"
        class="w-72 md:w-full mx-4 my-2 overflow-x-auto"
...
import CFileInput from '~/components/atom/CFileInput.vue'
...
function processCsv(
  ...
  let master_data_song: IObject = {} as any
  ...
  let statistics = ['<table class="md:w-full">']
  ...
  for (let i = 12; i >= 1; i--) {
    const row = stats[i]
    statistics.push('<tr class="bg-white even:bg-blue-200">')
  ...
  let list = ['<table class="md:w-full">']
  let list_header = `
    <thead class="bg-blue-400">
    <tr>
    <td> ☆ </td>
  ...
  for (const s of ret) {
    // console.log(s);
    if (check_12 && s['level'] !== '12') {
      continue
    }
    list.push('<tr class="bg-white even:bg-blue-200">')
...
function createStatisticsCell(row: any, name: any) {
...
export default defineComponent({
  ...
  setup() {
    ...
    watch(file, async () => {
      ...
      // const parseText = await new Promise((resolve) => {
      //   Papa.parse(csv, {
      //     complete: (results) => {
      //       errorMessage.value = ''
      ...
      //     },
      //     error: () => {
      //       errorMessage.value = 'CSV 入れて！！！！！！'
      //     },
      //   })
      // })
      const val = processCsv(csv, masterData.value, lv12.value)
      if (typeof val === 'string') {
        errorMessage.value = val
      } else {
        errorMessage.value = ''
        table.value = val.list
        statistics.value = val.statistics
      }
    })

    const masterData = useAsync(async () => {
      const masterRaw = (await $content(
        'master_sp_songs'
      ).fetch()) as IContentDocument
      return masterRaw.body.map(
        (m: {
          version_full: string
          title: string
          genre: string
          ...
          return {
            title: m.title,
            version: m.version_full,
            difficulty: m.difficulty_short,
            level: m.level,
            notes: m.notes,
            kaiden_average: m.kaiden_average,
    ...
    const lv12 = ref(false)
    const enable_heatmap = ref(false)
